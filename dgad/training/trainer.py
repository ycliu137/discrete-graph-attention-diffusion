...
def fit_dgad(
    ...
    *,
    ...
    if edge_rewire:
        ...
        data = (node_features, edge_index.to(device))

    if loss_adj > 0 and target_adj is None:
        num_nodes = node_features.shape[0]
        adj_edge = edge_index if edge_index is not None else None
        if adj_edge is None:
            raise ValueError("target_adj or edge_index required for adjacency loss")
        target_adj = edge_index_to_adj(adj_edge, num_nodes)

    target_adj = target_adj.to(device) if target_adj is not None else None
    ...
    for epoch in range(max_epochs):
        ...
        if loss_adj == 1.0:
            loss = F.binary_cross_entropy_with_logits(
                recon_adj, target_adj, reduction=loss_reduction
            )
        elif loss_adj == 0.0:
